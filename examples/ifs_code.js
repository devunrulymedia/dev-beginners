...
function drawImage(url, x, y) {
  ...
  var base = 'http://0.0.0.0:8000/dev-beginners/img/';
  _ctx.scale(-1, -1);
  var img = new Image();
  img.onload = function() {
...
function drawDelivery(x,y) {
  drawImage('ops.png',x,y);
}

function drawBuild(x,y) {
  drawImage('platform.png',x,y);
}

function drawMoving(x,y) {
  drawImage('moving.png',x,y);
}

function drawSell(x,y) {
  drawImage('sell.png',x,y);
}
